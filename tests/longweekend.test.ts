...
describe("Long Weekend Algorithm (PHT)", () => {
  describe("Natural long weekends (Fri/Mon)", () => {
    it("Friday holiday creates Fri–Sun (3-day weekend)", () => {
      const input: Holiday[] = [
        h("2025-08-29", "National Heroes Day (Observed)"),
      ]; // Fri
  ...
  describe("Suggested long weekends (Tue/Thu) full 4-day span", () => {
    it("Tuesday holiday → Mon leave → Sat–Tue", () => {
      const input: Holiday[] = [
        h("2025-12-30", "Rizal Day"),
        h("2025-12-31", "Last Day of the Year"),
        h("2026-01-01", "New Year's Day"),
      ];
      const out = computeLongWeekendsPHT(input);
      expect(out.map((it) => it.title)).toContain("Rizal Day");
      const rizalEntry = out.find((it) => it.title === "Rizal Day")!;
      expect(rizalEntry).toMatchObject({
        type: "suggested",
        startISO: toPHTMidnightISO("2025-12-27"),
        endISO: toPHTMidnightISO("2026-01-01"),
        durationLabel: "very long weekend",
      });
      expect(rizalEntry.suggestedLeaveISO).toBe(toPHTMidnightISO("2025-12-29"));
      expect(rizalEntry.extraHolidayISOs).toEqual([
        toPHTMidnightISO("2025-12-31"),
        toPHTMidnightISO("2026-01-01"),
      ]);
    });
